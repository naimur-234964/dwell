...
interface AdminBookingEditProps { // Changed from AdminPropertyEditProps
    booking: Booking; // Changed from property: Property & { address: Address; amenities: Amenity[]; property_images: PropertyImage[] };
}

export default function AdminBookingEdit({ booking }: AdminBookingEditProps) { // Changed from AdminPropertyEdit and removed amenities prop
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.bookings.update', booking.id)); // Changed from admin.properties.update to admin.bookings.update
    };
            ...
            <div className="p-4">
                <Card>
                    <CardHeader className="relative">
                        <CardTitle>Edit Booking (Admin)</CardTitle> {/* Changed from Edit Property to Edit Booking */}
                        <CardDescription>Editing: {booking.id}</CardDescription> {/* Changed from Editing: ${property.title} to Editing: ${booking.id} */}
                        <div className="absolute top-4 right-4">
                            <Link href="/admin/bookings"> {/* Changed from /admin/properties to /admin/bookings */}
