...
interface AdminPaymentIndexProps { // Changed from AdminPropertyIndexProps to AdminPaymentIndexProps
...
export default function AdminPaymentIndex({ payments }: AdminPaymentIndexProps) { // Changed from AdminPropertyIndex to AdminPaymentIndex and properties to payments
    ...
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this payment?')) { // Changed from property to payment
            router.delete(`/admin/payments/${id}`); // Changed from /admin/properties to /admin/payments
        }
    };

    return (
        <AppLayout>
            <Head title="Admin Payments" /> {/* Changed from Admin Properties to Admin Payments */}
            <div className="p-4">
                <div className="flex justify-between items-center mb-4">
                    <h1 className="text-2xl font-bold">Admin Payments</h1> {/* Changed from Admin Properties to Admin Payments */}
                    <Link href="/admin/payments/create"> {/* Changed from /admin/properties/create to /admin/payments/create */}
                ...
                <div className="mb-4">
                    <Input
                        type="text"
                        placeholder="Search payments..." // Changed from Search properties to Search payments
                        value={searchTerm}
                            ...
                            {filteredPayments.map((payment) => (
                                <TableRow key={payment.id}>
                                    ...
                                    <TableCell className="flex gap-2">
                                        <Link href={`/admin/payments/${payment.id}`}> {/* Changed from /admin/properties to /admin/payments */}
                                            <Button variant="outline" size="icon"><Eye className="h-4 w-4" /></Button>
                                        </Link>
                                        <Link href={`/admin/payments/${payment.id}/edit`}> {/* Changed from /admin/properties to /admin/payments */}
