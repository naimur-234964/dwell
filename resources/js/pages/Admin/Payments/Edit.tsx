...
interface AdminPaymentEditProps { // Changed from AdminPropertyEditProps
    payment: Payment; // Changed from property: Property & { address: Address; amenities: Amenity[]; property_images: PropertyImage[] };
}

export default function AdminPaymentEdit({ payment }: AdminPaymentEditProps) { // Changed from AdminPropertyEdit and removed amenities prop
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.payments.update', payment.id)); // Changed from admin.properties.update to admin.payments.update
    };
            ...
            <div className="p-4">
                <Card>
                    <CardHeader className="relative">
                        <CardTitle>Edit Payment (Admin)</CardTitle> {/* Changed from Edit Property to Edit Payment */}
                        <CardDescription>Editing: {payment.id}</CardDescription> {/* Changed from Editing: ${property.title} to Editing: ${payment.id} */}
                        <div className="absolute top-4 right-4">
                            <Link href="/admin/payments"> {/* Changed from /admin/properties to /admin/payments */}
