...
interface CustomerReviewEditProps { // Changed from HostPropertyEditProps
    review: Review; // Changed from property: Property & { address: Address; amenities: Amenity[]; property_images: PropertyImage[] };
}

export default function CustomerReviewEdit({ review }: CustomerReviewEditProps) { // Changed from HostPropertyEdit and removed amenities prop
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('customer.reviews.update', review.id)); // Changed from host.properties.update to customer.reviews.update
    };
            ...
            <div className="p-4">
                <Card>
                    <CardHeader className="relative">
                        <CardTitle>Edit Review (Customer)</CardTitle> {/* Changed from Edit Property to Edit Review */}
                        <CardDescription>Editing: {review.id}</CardDescription> {/* Changed from Editing: ${property.title} to Editing: ${review.id} */}
                        <div className="absolute top-4 right-4">
                            <Link href="/customer/reviews"> {/* Changed from /host/properties to /customer/reviews */}
