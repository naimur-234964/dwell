...
interface CustomerBookingEditProps { // Changed from HostPropertyEditProps
    booking: Booking; // Changed from property: Property & { address: Address; amenities: Amenity[]; property_images: PropertyImage[] };
}

export default function CustomerBookingEdit({ booking }: CustomerBookingEditProps) { // Changed from HostPropertyEdit and removed amenities prop
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('customer.bookings.update', booking.id)); // Changed from host.properties.update to customer.bookings.update
    };
            ...
            <div className="p-4">
                <Card>
                    <CardHeader className="relative">
                        <CardTitle>Edit Booking (Customer)</CardTitle> {/* Changed from Edit Property to Edit Booking */}
                        <CardDescription>Editing: {booking.id}</CardDescription> {/* Changed from Editing: ${property.title} to Editing: ${booking.id} */}
                        <div className="absolute top-4 right-4">
                            <Link href="/customer/bookings"> {/* Changed from /host/properties to /customer/bookings */}
