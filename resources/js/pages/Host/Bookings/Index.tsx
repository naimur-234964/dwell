...
interface HostBookingIndexProps { // Changed from HostPropertyIndexProps to HostBookingIndexProps
...
export default function HostBookingIndex({ bookings }: HostBookingIndexProps) { // Changed from HostPropertyIndex to HostBookingIndex and properties to bookings
    ...
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this booking?')) { // Changed from property to booking
            router.delete(`/host/bookings/${id}`); // Changed from /host/properties to /host/bookings
        }
    };
        ...
        <AppLayout>
            ...
            <div className="p-4">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <Link href="/host/bookings/create"> {/* Changed from /host/properties/create to /host/bookings/create */}
                            ...
                            {filteredBookings.map((booking) => (
                                <TableRow key={booking.id}>
                                    ...
                                    <TableCell className="flex gap-2">
                                        <Link href={`/host/bookings/${booking.id}`}> {/* Changed from /host/properties to /host/bookings */}
                                            <Button variant="outline" size="icon"><Eye className="h-4 w-4" /></Button>
                                        </Link>
                                        <Link href={`/host/bookings/${booking.id}/edit`}> {/* Changed from /host/properties to /host/bookings */}
